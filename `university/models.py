...
class StudentRequest(models.Model):
    ...
    start_date = models.DateField(null=True, blank=True, help_text="Deferment / leave start date")
    end_date = models.DateField(null=True, blank=True, help_text="Expected deferment / leave end date (resumption date)")
    status = models.CharField(max_length=15, choices=Status.choices, default=Status.PENDING, db_index=True)
...
class AttachmentAssessment(models.Model):
    ...
    organization_suitability_score = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal("8.00"))
    student_attendance_score = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal("13.00"))
    technical_skills_score = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal("30.00"))
    logbook_maintenance_score = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal("17.00"))
    oral_presentation_score = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal("17.00"))
    total_score = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal("85.00"))
...
class SystemPermission(models.Model):
    code = models.CharField(max_length=60, unique=True, db_index=True, help_text="Machine identifier e.g. 'exams.approve_senate'")
    name = models.CharField(max_length=120)
    module = models.CharField(max_length=60, db_index=True, help_text="System subsystem e.g. 'Examinations', 'Academics', 'Finance'")
    description = models.TextField(blank=True, default="")
...
class StaffRole(models.Model):
    ...
    description = models.TextField(blank=True, default="")
    color = models.CharField(max_length=20, default="#6C5CE7", help_text="Hex code for role badge e.g. '#00b894'")
...
class StaffRoleAssignment(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="staff_role_assignments")
    role = models.ForeignKey(StaffRole, on_delete=models.CASCADE, related_name="assignments")
    department = models.ForeignKey(Department, on_delete=models.SET_NULL, null=True, blank=True, related_name="staff_role_assignments")
...
class UserPermissionOverride(models.Model):
    ...
    permission = models.ForeignKey(SystemPermission, on_delete=models.CASCADE, related_name="user_overrides")
    override_type = models.CharField(max_length=10, choices=OverrideType.choices, default=OverrideType.GRANT)
    reason = models.TextField(blank=True, default="")
...
class DocumentReleaseControl(models.Model):
    ...
    term = models.ForeignKey(
        "AcademicTerm", on_delete=models.CASCADE, null=True, blank=True, related_name="document_controls",
    ...
    require_financial_clearance = models.BooleanField(default=False)
    max_allowed_fee_balance = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal("0.00"))
    require_senate_approval = models.BooleanField(default=False)
...
class AdmissionDocumentTemplate(models.Model):
    ...
    fee_schedule_instructions = models.TextField(blank=True, default="")
...
class IssuedAdmissionDocument(models.Model):
    ...
    rendered_context = models.JSONField(default=dict, blank=True)
...
class ApplicationCustomField(models.Model):
    ...
    choices_list = models.TextField(blank=True, default="")
